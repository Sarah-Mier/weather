...
$("#current-conditions-text").html(Math.round(response.currently.temperature) + "°F and " + response.currently.summary);
   $(".current").fadeIn(500)

    $(".forecast").empty();
    for (let x=1; x<7; x++) {
    let forecastWeek = response.daily.data[x];
    let html = '<div class="col">';
      html = html + '<h3>' + icon(forecastWeek.icon) + '</h3>';
      html = html + '<h4>' + Math.round(forecastWeek.apparentTemperatureLow)+ " | " + Math.round(forecastWeek.apparentTemperatureHigh) + '</h4>';
      html = html + '<h5>' + forecastWeek.summary + '</h5>';
      html = html + '</div>';
